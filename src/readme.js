...
function generateMethodsTable() {
    ...
    return [
        '| Name | Logo | Colors |',
        '| ---- | ---- | ------ |',
        ...methods.map(i => `| ${i.name} | ![${i.name} Logo](https://raw.githubusercontent.com/College-Canine/financial/master/logos/method/${i.logo}) | ${i.colors.map(j => `![${j}](https://placehold.co/15x15/${j.slice(1)}/${j.slice(1)}.png)`).join('')} |`)
    ].join('\n')
